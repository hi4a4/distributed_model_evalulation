...
def predict_on_video(
        model_weights, concepts, filename, video_capture, collections=None, start_frame=None, end_frame=None):
    print("Initializing Model")
    model = init_model(model_weights)

    printing_with_time("Predicting")
    results = predict_frames(video_capture, model, concepts, collections, start_frame, end_frame)
    if (results.empty):
...
def init_model(model_path):
    ...
    model = load_model(config.MODEL_WEIGHTS_FOLDER + "/" + model_path, backbone_name='resnet50')
    model = convert_model(model)
    return model


def print_predict_progress(current_print, total_frames, frame_num):
    current_print_len = len(current_print)
    progress = round(frame_num / total_frames, 3)
    current_print = f'Predicting in Progress: {progress}'
    print('\b' * current_print_len, end='', flush=True)
    print(current_print, end='')
    return current_print


def predict_frames(video_capture, model, concepts, collections=None, start_frame=None, end_frame=None):
    ...
    end_frame = end_frame if end_frame else int(video_capture.get(cv2.CAP_PROP_FRAME_COUNT))
    start_frame = start_frame if start_frame else 0
    total_frames = end_frame - start_frame
    print(f'total frames predicting on {total_frames}')
    video_frames = []
    ...
    for frame_num in range(start_frame, end_frame):
        current_print = print_predict_progress(current_print, total_frames, frame_num - start_frame)
        ...
        if (frame_num - start_frame) % config.NUM_FRAMES == 0:
